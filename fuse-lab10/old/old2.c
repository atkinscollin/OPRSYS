...
const char SECRET_ID[] = "87";

bool HasAccess(struct bb_state* bb_data) {
	if((bb_data->uid != getuid()) || (bb_data->uid[1] != SECRET_ID[1])) {
		log_msg("\n user tried to illegaly access file %s");
		return false;
	}
	return true;
}
...
int bb_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
   ...
   /*honestly not sure where else to put the encryption */
   if (HasAccess(BB_DATA) == true) {
...
int main(int argc, char *argv[]) {
   ...
   if ((getuid() == 0) || (geteuid() == 0)) {
	...
	// Checks arg 3(uid) to see if it matches the secret id
	//if((uid[0] != SECRET_ID[0]) || (uid[1] != SECRET_ID[1])) {bb_usage();}
   ...
   fprintf(stderr, "fuse_main returned %d\n", fuse_stat);
	...
	if(HasAccess(BB_DATA) == false) {
		fprintf(stderr, "has access true %d\n", fuse_stat);
		bb_usage();
	}
   return fuse_stat;
}
